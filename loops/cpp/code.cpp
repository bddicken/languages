...
int main (int argc, char* argv[])
{
    ...
    for (int const i : std::ranges::iota_view{0, 10000})      // 10k outer loop iterations
    {
        for (int const j : std::ranges::iota_view{0, 100000}) // 100k inner loop iterations, per outer loop iteration
            array[i] += j % mod;                              // Simple sum
        array[i] += picked_number;                            // Add a random value to each element in array
    }
    std::print("{}\n", array[picked_number]);                 // Print out a single element from the array
}
